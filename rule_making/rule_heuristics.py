# This file is derived from "rule_heuristics.py" from the repository:
# GitHub: https://github.com/jobregon1212/rulecosi/blob/9928a4a586ec90f35021df80d01c1f5347b037ab/rulecosi/rule_heuristics.py
# Original Author: jobregon1212
# Description: Portions of code have been copied to implement rule heuristics handling.
...
class RuleHeuristics:
    ...
    def get_conditions_heuristics(self, conditions, not_cov_mask=None):
        ...
        if len(conditions) == 0:
            return (
                self.get_training_heuristics_dict(not_cov_mask=not_cov_mask),
                not_cov_mask,
            )

        b_array_conds = [
            reduce(
                operator.and_, [self._cond_cov_dict[i][cond[0]] for cond in conditions]
            )
        ...
        class_cov_count = [
            self.bitarray_.get_number_ones(b_array_conds[i])
            for i in range(self.n_classes)
        ]
        ...
        heuristics_dict["conf"] = [
            class_count / cov_count for class_count in class_cov_count
        ]
        heuristics_dict["supp"] = [
            class_count / self.X.shape[0] for class_count in class_cov_count
        ]

        return heuristics_dict, not_cov_mask

    def compute_rule_heuristics(
        self, ruleset, not_cov_mask=None, sequential_covering=False, recompute=False
    ):
        ...
        if recompute:
            for rule in ruleset:
                heuristics_dict, _ = self.get_conditions_heuristics(rule.A)
    ...
    def _compute_training_bit_sets(self):
        ...
        """
        training_bit_set = [
            self.bitarray_.get_array(self.y == self.classes_[i])
            for i in range(self.n_classes)
        ]
        training_bit_set.append(reduce(operator.or_, training_bit_set))
    ...
    def _compute_condition_bit_sets(self):
        ...
        for cond_id, cond in self.condition_map.items():
            ...
            for i in range(self.n_classes):
                cond_cov_dict[i][cond_id] = (
                    cond_coverage_bitarray & self.training_bit_sets[i]
                )
    ...
    def rule_is_accurate(self, rule, not_cov_samples):
        ...
        local_not_cov_samples = self.set_rule_heuristics(rule, not_cov_samples)
    ...
    def get_training_heuristics_dict(self, not_cov_mask=None):
        ...
        if self.training_heuristics_dict is None:
            cov_count = self.bitarray_.get_number_ones(self.training_bit_sets[-1])
            class_cov_count = [
                self.bitarray_.get_number_ones(self.training_bit_sets[i])
                for i in range(self.n_classes)
            ]
            coverage = cov_count / self.X.shape[0]
            train_heur_dict = {
                "cov_set": self.training_bit_sets,
                "cov": coverage,
                "cov_count": cov_count,
        ...
        if not_cov_mask is None:
            ...
            masked_training_heuristics = [
                b_array_measure & not_cov_mask
                for b_array_measure in self.training_bit_sets
            ]
            cov_count = self.bitarray_.get_number_ones(masked_training_heuristics[-1])
            class_cov_count = [
    ...
    def combine_heuristics(self, heuristics1, heuristics2):
        ...
        class_cov_count = [
            self.bitarray_.get_number_ones(cov_set[i]) for i in range(self.n_classes)
        ]
        ...
        return {
            ...
            "class_cov_count": class_cov_count,
            "conf": [class_count / cov_count for class_count in class_cov_count],
            "supp": [class_count / self.X.shape[0] for class_count in class_cov_count],
        }
    ...
    def set_rule_heuristics(self, rule, mask):
        ...
        if cov_count == 0:
            ...
            rule.supp = 0.0
            rule.cov = 0.0
            return self.bitarray_.get_complement(mask_cov_set[-1], self.ones) & mask

        else:
            class_cov_count = [
                self.bitarray_.get_number_ones(mask_cov_set[i])
                for i in range(self.n_classes)
            ]
